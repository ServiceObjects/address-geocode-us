...
async function GetBestMatchV4ClientGo(licenseKey, isLive) {
    console.log("\n----------------------------------------------");
    console.log("Address Geocode US - GetBestMatchV4 - REST SDK");
    console.log("----------------------------------------------");
    ...
    try {
        const response = await GetBestMatchV4Client.invoke(address, city, state, postalCode, licenseKey, isLive, timeoutSeconds);
        console.log("\n* Geocode Info *\n");
        if (response)
        {
