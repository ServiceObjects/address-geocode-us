...
/**
 * <summary>
 * A class that provides functionality to call the ServiceObjects Address Geocode US (AGUS) SOAP service's GetDistanceToWater endpoint,
...
class GetDistanceToWaterSoap {
    /**
     * <summary>
     * Initializes a new instance of the GetDistanceToWaterSoap class with the provided input parameters,
    ...
    /**
     * <summary>
     * Asynchronously calls the GetDistanceToWater SOAP endpoint, attempting the primary endpoint
     * first and falling back to the backup if the response is invalid (Error.Number == '4') in live mode
     * or if the primary call fails.
     * </summary>
     * <returns type="Promise<DistanceToWaterInfoResponse>">A promise that resolves to a DistanceToWaterInfoResponse object containing distance details or an error.</returns>
     ...
     * creating a client and processing the response into a DistanceToWaterInfoResponse object.
     * </summary>
     * <param name="wsdlUrl" type="string">The WSDL URL of the SOAP service endpoint (primary or backup).</param>
     * <param name="args" type="Object">The arguments to pass to the GetDistanceToWater method.</param>
     * <returns type="Promise<DistanceToWaterInfoResponse>">A promise that resolves to a DistanceToWaterInfoResponse object containing the SOAP response data.</returns>
    ...
    _callSoap(wsdlUrl, args) {
        return new Promise((resolve, reject) => {
            soap.createClient(wsdlUrl, { timeout: this.timeoutSeconds * 1000 }, (err, client) => {
                ...
                client.GetDistanceToWater(args, (err, result) => {
                    const rawData = result?.DistanceToWaterInfo;
                    try {
                        if (!rawData) {
     ...
     * </summary>
     * <param name="response" type="DistanceToWaterInfoResponse">The DistanceToWaterInfoResponse object to validate.</param>
